...
path_mc = {
        2017: Data2017['data'].path[0],
        2018: Data2018['data'].path[0],
        }
...
def studycuts(year, treename, weight=None, ptName="ph_et", inputFileName=None, outputFileName=outdir+'histos.root'):
    ...
    if weight:
        ...
        Weight[0]=1
    TAG_PT = np.zeros(1,dtype="float32")
    TAG_ETA = np.zeros(1,dtype="float32")
    MASS = np.zeros(1,dtype="float32")
    tree.SetBranchAddress("tag_Ele_pt", TAG_PT)
    tree.SetBranchAddress("tag_sc_abseta", TAG_ETA)
    tree.SetBranchAddress("pair_mass", MASS)
    ##### loop tree
    ...
    with tqdm(total=nentries) as pbar:
        for iEntry in range(nentries):
            ...
            if abs(TAG_ETA[0])>1.4442: continue
            if TAG_PT[0]<50: continue
            if MASS[0]<80 or MASS[0]>100: continue  ### tighter cut on data
            for cut in all_cuts:
